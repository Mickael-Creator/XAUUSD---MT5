...
class COTDataManager:
    ...
    def _fetch_from_cftc_socrata(self):
        """
        Fetch from CFTC Socrata Open Data API (Legacy Futures Only report)
        Free, no API key required, reliable
        """
        try:
            url = "https://publicreporting.cftc.gov/resource/jun7-fc8e.json"

            # Fetch history for percentile calculation
            params = {
            ...
            if response.status_code != 200:
                print(f"[COT] CFTC Socrata returned {response.status_code}")
                return None

            data = response.json()

            if not data:
                print("[COT] No data from CFTC Socrata")
                return None

            latest = data[0]

            # Non-Commercial = Large Speculators (equivalent to Managed Money for sentiment)
            mm_long = int(float(latest.get('noncomm_positions_long_all', 0)))
            mm_short = int(float(latest.get('noncomm_positions_short_all', 0)))
            ...
            print(f"[COT] CFTC Socrata OK: date={report_date}, net={mm_net:+,}, percentile={percentile:.1f}%")
            ...
            print(f"[COT] CFTC Socrata error: {e}")
            return None
